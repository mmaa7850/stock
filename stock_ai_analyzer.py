...
def calculate_technical_from_hist(hist: pd.DataFrame, realtime_price: float) -> dict:
    ...
    ma5  = round(float(close_all.rolling(5).mean().iloc[-1]),  2)   # v5.2 新增：極短線動能追蹤
    ...
    return {
        ...
        "volume_ratio":          volume_ratio,           # 今日量能 / 60 日均量
...
def build_analysis_context(
    ...
    ma5  = technical.get("ma5",  technical["ma20"])            # v5.2 新增：極短線動能指標
    ma20 = technical["ma20"]
    ma60 = technical["ma60"]
    rsi  = technical["rsi"]

    # ── 量能比計算（後續洗盤偵測要用）──────────────────────────────────────
    vol_ratio = technical.get(
        "volume_ratio",
        technical["current_volume"] / technical["avg_volume_60d"]
        if technical["avg_volume_60d"] > 0 else 1.0
    )

    # ── v5.2 核心：MA20 乖離率 & 動態市場型態分類器（A / B / C）──────────────
    #
    # 設計哲學：
    #   不同市場型態需要截然不同的操作策略。
    #   把型態判斷硬編碼在 Python 端（而非讓 AI 自行判斷），
    #   確保 AI 拿到的是明確的「型態標籤 + 對應策略路徑」，
    #   而不是讓它自己從均線數字推理型態（容易推錯）。
    #
    #   型態 A（極度強勢）：死等 MA20 必踏空 → 強勢股動能追擊策略
    #   型態 B（健康多頭）：常規防洗盤蹲點 → 防洗盤三重濾網策略
    #   型態 C（低位潛伏）：均線糾結或跌破均線 → 左側交易價值重估策略
    ma20_deviation = round((price - ma20) / ma20 * 100, 2)

    # 型態 A 乖離率觸發門檻：美股高價股無漲跌幅限制，漲幅更猛，門檻調低
    regime_a_threshold = 8.0 if is_us_high_price else 12.0

    if price > ma5 > ma20 > ma60 and ma20_deviation > regime_a_threshold:
        regime       = "A"
        regime_label = "🚨 型態 A：極度強勢 / 踏空熱門股"
        regime_desc  = (
            f"均線完美多頭強烈發散（{cs}{price} > MA5 {cs}{ma5} > MA20 {cs}{ma20} > MA60 {cs}{ma60}），"
            f"MA20 乖離率 {ma20_deviation:+.2f}%（已超過門檻 {regime_a_threshold}%）。"
            "此時死等均線回踩必將踏空，策略強制切換為【強勢股動能追擊】。"
        )
    elif price > ma20 and ma20 > ma60:
        regime       = "B"
        regime_label = "📈 型態 B：健康多頭 / 標準波段股"
        regime_desc  = (
            f"多頭排列（{cs}{price} > MA20 {cs}{ma20} > MA60 {cs}{ma60}），"
            f"MA20 乖離率 {ma20_deviation:+.2f}%，在合理蹲點回踩區間。"
            "採用防洗盤三重濾網與常規蹲點策略。"
        )
    else:
        regime       = "C"
        regime_label = "🔍 型態 C：低位潛伏 / 景氣轉機股"
        regime_desc  = (
            f"均線糾結或股價跌破 MA20（{cs}{price} vs MA20 {cs}{ma20}，"
            f"乖離率 {ma20_deviation:+.2f}%）。"
            "切換為左側交易 / 價值重估策略，尋找打底進場點。"
        )
    ...
    if position["holds"]:
        ...
        # ── v5.4：人性化複合風控 - 硬停損數字（所有持倉情境均計算）────────────
        hard_stop_6pct  = round(price * 0.94, 2)   # 現價 -6%（Hard Stop）
        cost_floor_3pct = round(cost * 0.97, 2)    # 成本 -3%（低催化劑觸底線）
        cost_floor_5pct = round(cost * 0.95, 2)    # 成本 -5%（高催化劑容忍極限）
        hard_stop_note  = (
            f"\n\n  【v5.4 人性化複合風控數字（AI 強制參照）】"
            f"\n  🚨 Hard Stop（現價 -6%）：{cs}{hard_stop_6pct}"
            f"（跌破此線不問理由先出清 50%+，凌駕一切均線 / 題材考量）"
            + (
                f"\n  ⚠ 成本下潛觸底線（催化劑弱，-3%）：{cs}{cost_floor_3pct}"
                f"\n  ⚠ 成本下潛容忍極限（催化劑強，-5%）：{cs}{cost_floor_5pct}"
                f"\n  ※ AI 依本次催化劑強度（★1-4 顆）在 3%~5% 範圍動態選執行線；"
                f"跌破即強制先減碼，即使防洗盤三重濾網未全成立也必須執行"
                if pnl_pct > 0 else ""
            )
        )
        breakeven_note += hard_stop_note
...
▌【v4.0 洗盤偵測分析】（防止被主力洗出場）
  ...
  ⚙ AI 分析師核心任務（持有者）：
  ⓪ 【最高優先：硬停損 Hard Stop】股價若從現價 {cs}{price} 回落超過 6%（跌至 {cs}{hard_stop_6pct}）
     → 不問理由先出清 50%+，此規則凌駕防洗盤三重濾網與一切題材面，無例外
  ① 獲利持倉：先確認「成本下潛容忍底線」（依催化劑強度動態選 {cs}{cost_floor_3pct} 至 {cs}{cost_floor_5pct}）
     跌破此線即強制先減碼 50%，不得以「主力洗盤」為由讓使用者繼續凹單
  ② 通過 ⓪① 確認後，再執行防洗盤三重濾網（條件 A/B/C）決定是否全數出場
  ...
  ⑤ 【8% 上限鐵律】禁止給出距現價 {cs}{price} 超過 8%（即 {cs}{round(price * 0.92, 2)} 以下）的均線防守點"""
...
▌【使用者持倉資訊】
  ...
  ⚙ AI 分析師任務（未持有者情境，依市場型態切換策略）：
  {'【型態 A 強勢股：禁止給死等均線的保守建議，改輸出強勢動能追擊三步驟】' if regime == 'A' else ''}
  {'• 第一步：縮量回踩 MA5（' + cs + str(ma5) + '）蹲點試單（10~15% 輕倉）' if regime == 'A' else '• 積極型進場點：MA20（' + cs + str(ma20) + '）附近，說明觸發條件（縮量回測才入）'}
  {'• 第二步：放量突破當日最高點，收盤守住後加碼至 30~50% 倉位' if regime == 'A' else '• 保守型進場點：MA60（' + cs + str(ma60) + '）或近期低點（' + cs + str(technical['low_60d']) + '）'}
  {'• 停損：鎖在突破當天 K 線低點，跌破不猶豫出場（高賠率但必守紀律）' if regime == 'A' else '• 若 RSI 目前 > 70，明確說明不追高，等哪個回踩位再進場'}
  {'  → 重要：型態 A 強勢股禁止給出「等 MA20/MA60 再進場」的建議，此時死等必踏空' if regime == 'A' else '• 若為強勢題材股且動能未衰退，可建議少量零股試單蹲點'}"""
...
▌【v5.2 動態市場型態分析】（Python 端預判，AI 直接使用此分類，禁止自行覆蓋）
  AI 判定型態：{regime_label}
  均線快照：MA5 {cs}{ma5}  ｜  MA20 {cs}{ma20}  ｜  MA60 {cs}{ma60}
  MA20 乖離率：{ma20_deviation:+.2f}%（型態 A 觸發門檻：>{regime_a_threshold}%）
  判定邏輯：{regime_desc}
...
▌技術面數據（{('即時修正後重算' if technical.get('price_overridden') else 'Yahoo 延遲快照')}）
  當前股價：{cs}{price}
  MA5：{cs}{ma5}  ｜  MA20：{cs}{ma20}  ｜  MA60：{cs}{ma60}  ｜  RSI(14)：{rsi}
  MA20 乖離率：{ma20_deviation:+.2f}%（型態判定關鍵數據）
  均線研判：{ma_summary}
  RSI 研判：{rsi_signal}
  量能研判：{vol_signal}
...
def analyze_with_claude(
    ...
    # ── v5.2 市場型態：在 analyze_with_claude 端複算（與 build_analysis_context 邏輯一致）──
    _price       = technical["current_price"]
    _ma5         = technical.get("ma5", technical["ma20"])
    _ma20        = technical["ma20"]
    _ma60        = technical["ma60"]
    _is_tw       = fundamental.get("currency") == "TWD"
    _is_us_hp_r  = (not _is_tw) and _price >= 50
    _dev         = round((_price - _ma20) / _ma20 * 100, 2)
    _regime_a_th = 8.0 if _is_us_hp_r else 12.0

    if _price > _ma5 > _ma20 > _ma60 and _dev > _regime_a_th:
        _regime       = "A"
        _regime_label = "🚨 型態 A：極度強勢 / 踏空熱門股"
    elif _price > _ma20 and _ma20 > _ma60:
        _regime       = "B"
        _regime_label = "📈 型態 B：健康多頭 / 標準波段股"
    else:
        _regime       = "C"
        _regime_label = "🔍 型態 C：低位潛伏 / 景氣轉機股"

    _ma20_dev_str = f"{_dev:+.2f}%"

    # ── v5.4：人性化複合風控變數 ──────────────────────────────────────────────
    _cost_r           = position.get("cost_basis") if position["holds"] else None
    _pnl_pct_r        = position.get("unrealized_pnl_pct", 0) if position["holds"] else 0
    _hard_stop_6pct_r = round(_price * 0.94, 2) if position["holds"] else None
    _cost_floor_3pct_r = round(_cost_r * 0.97, 2) if _cost_r else None
    _cost_floor_5pct_r = round(_cost_r * 0.95, 2) if _cost_r else None
    _price_8pct_floor  = round(_price * 0.92, 2)          # 防守線距現價 8% 上限

    _hs_str  = f"{cs}{_hard_stop_6pct_r}"  if _hard_stop_6pct_r  else "（未持有，不適用）"
    _cf3_str = f"{cs}{_cost_floor_3pct_r}" if _cost_floor_3pct_r else "（未持有，不適用）"
    _cf5_str = f"{cs}{_cost_floor_5pct_r}" if _cost_floor_5pct_r else "（未持有，不適用）"

    # 輸出格式用：風險防守區塊首部（有持倉才插入 Hard Stop / 成本底線行）
    if position["holds"] and _hard_stop_6pct_r:
        _risk_header_lines = (
            f"🚨 **Hard Stop（現價 -6%，無條件硬停損）**：{_hs_str}\n"
            f"　↳ 跌破此線不問理由先出清 50%~100%，凌駕均線 / 題材 / 洗盤判斷"
        )
        _cost_floor_lines = (
            f"\n⚠ **成本下潛底線（依催化劑動態）**："
            f"{_cf3_str}（-3%，弱催化劑）至 {_cf5_str}（-5%，強催化劑）\n"
            f"　↳ 跌破此線即使洗盤三重濾網未全成立也先減碼 50%"
        ) if _cost_floor_3pct_r and _pnl_pct_r > 0 else ""
    else:
        _risk_header_lines = ""
        _cost_floor_lines  = ""

    # 型態 A 強勢追擊三步驟格式（預建，插入系統提示模板）
    if _regime == "A":
        _regime_ops_guide = (
            f"（型態 A 強制三步驟格式，每步驟必須附具體股價數字）\n"
            f"- **第一步（輕倉試單 10~15%）**："
            f"等縮量（< 均量 0.7 倍）回踩 MA5（{cs}{_ma5}）不破，"
            f"出現縮量紅 K 確認守住後入場；此為最低風險試探買點\n"
            f"- **第二步（動能加碼 30~50%）**："
            f"放量（≥ 均量 1.5 倍）突破當日最高點 [{cs}填入當日最高點]，"
            f"收盤守住確認後加碼；若直接爆量突破可不等 MA5 蹲點直接入場\n"
            f"- **第三步（高紀律停損）**："
            f"停損鎖在突破當天 K 線低點 [{cs}填入突破 K 棒低點]，跌破不猶豫出場"
        )
    elif _regime == "B":
        _regime_ops_guide = (
            "- [第一步：具體行動 + 觸發條件或價格]\n"
        ...
        _regime_ops_guide = (
            "- **第一步（觀察倉 20~30%）**：[利多催化劑確認有效後建立觀察倉，不一次進滿]\n"
            "- **第二步（加碼 50~70%）**：[底部型態確認，放量突破整理區後加碼]\n"
            "- **第三步（停損）**：[跌破近期顯著低點或催化劑被證偽，果斷出場]"
        )
...
══════════════════════════════════════════════
二、動態市場型態分類器（型態決定策略，鐵律）
══════════════════════════════════════════════
Context 已由 Python 端預判好「AI 判定市場型態」標籤，直接使用此分類，切換對應策略。
每種型態的操作邏輯截然不同，嚴禁混用策略或忽略型態標籤。

【型態 A：極度強勢 / 踏空熱門股】
  觸發條件：股價 > MA5 > MA20 > MA60，且 MA20 乖離率 > 12%（台股）/ > 8%（美股高價股）

  ❌ 嚴格禁止（違反以下任一 = 報告廢文）：
    - 給出「等回測 MA20 或 MA60 再進場」的無效保守建議
    - 因 Trailing P/E 看起來高就喊「估值昂貴，建議等待」
    - 給任何「等跌再買」的廢話，強勢主升浪死等必踏空

  ✅ 必須輸出【強勢股動能追擊策略】三步驟（每步驟必附具體股價數字）：
    第一步（輕倉試單 10~15%）：縮量（< 均量 0.7 倍）回踩 MA5 不破，出現縮量紅 K 確認守住後買入
    第二步（動能加碼 30~50%）：放量（≥ 均量 1.5 倍）突破當日最高點，收盤守住確認後加碼
    第三步（高紀律停損）：停損鎖在突破當天 K 線低點，跌破後不猶豫出場

【型態 B：健康多頭 / 標準波段股】
  觸發條件：股價 > MA20 > MA60，MA20 乖離率在合理回踩蹲點區間

  策略：採用防洗盤三重濾網（詳見下方規則三），分批蹲點布局
    - 第一買點：MA20 附近縮量回踩，配合量縮確認守穩後入場
    - 第二保守買點：MA60 附近等量縮止跌後入場

【型態 C：低位潛伏 / 景氣轉機股】
  觸發條件：均線糾結，或股價跌破 MA20，但 Forward P/E 極低 + 近期有重大利多催化劑

  策略：左側交易 / 價值重估策略
    - 禁止過度悲觀或建議全倉做空
    - 第一批（觀察倉 20~30%）：催化劑確認有效後建立，不急著進滿
    - 第二批（加碼至 50~70%）：底部型態確認（W 底 / 放量突破整理區）後加碼
    - 停損：跌破近期顯著低點或催化劑被證偽

══════════════════════════════════════════════
三、防洗盤三重濾網（核心鐵律，持有者適用）
══════════════════════════════════════════════
...
══════════════════════════════════════════════
四、人性化複合風控模型（v5.4 全面升級）
══════════════════════════════════════════════
核心信條：
「風控線不是均線，是保護人類心理能接受的最大本金損失邊界。」
「再強的題材，也不能讓獲利單超過人類短線心理承受極限，Hard Stop 是最後尊嚴。」

【鐵律：防守線距離上限（所有情境強制，不得違反）】
嚴格禁止在報告中給出距離「使用者現價 {cs}{_price}」超過 8% 以下的防守點。
→ 任何防守線必須在 {cs}{_price_8pct_floor}（現價 -8%）以上。
→ 若 MA20 或 MA60 低於此門檻，忽略這些均線，改依下方規則一/二設定防守線。
→ 例：現價 4,310，禁止叫使用者等到 3,965 以下才停損，此為荒謬的「叫人吐光所有獲利」指令。

─────────────────────────────────────────────
規則一（最高優先，適用一切持倉情境）：
🚨 Hard Stop 硬停損（現價 -6% = {_hs_str}）
─────────────────────────────────────────────
觸發條件：股價從「使用者輸入的即時現價」回落超過 6%，跌破 {_hs_str}。
執行方式：不管 MA20/MA60 在哪、不管題材有多強、不管是否符合洗盤三重濾網，
          立刻建議使用者「先出售 50%~100% 持股」，鎖住資金安全後再評估是否補回。
嚴禁例外：即使判定「主力洗盤」「外資目標價仍高」，也不能讓使用者凹過 Hard Stop。
          超過 6% 的回落已超出人類短線心理承受極限，任何理性交易員必須先止血再說。
...
─────────────────────────────────────────────
Context 已預算：
  ⚠ 成本下潛 3%（低催化劑觸底線）= {_cf3_str}
  ⚠ 成本下潛 5%（高催化劑容忍極限）= {_cf5_str}

【AI 必須依本次情報催化劑強度動態選執行線】：
  ★★★★ 極強（多家外資集體調升 / 供需缺口爆發）→ 容忍至成本 -5%，執行線 = {_cf5_str}
  ★★★☆ 強（單一大幅調升 / 板塊帶動）           → 容忍至成本 -4%（自算：成本 × 0.96）
  ★★☆☆ 中性（零散新聞 / 無明確催化劑）         → 容忍至成本 -3%，執行線 = {_cf3_str}
  ★☆☆☆ 負面（降評 / 財報下修）                 → 容忍至成本 -2%（自算：成本 × 0.98）

【觸發與執行】：
一旦股價跌破「對應容忍線」：
  → 即使防洗盤三重濾網（規則三）尚未完全成立
  → 即使外資目標價仍高、題材仍在
  → 必須判定「短線交易假設已被市場否定，先認錯，保住本金」
  → 建議使用者「先減碼 50%~100%，等盤面穩定後重新評估進場時機」

─────────────────────────────────────────────
規則三（規則一/二均未觸發時）：保本防守線鐵律
─────────────────────────────────────────────
若持倉處於獲利狀態，且股價尚在 Hard Stop 與成本容忍底線之上：

  ① 短線防守位不得低於 Context 中的「保本防守線」。
     （Context 已依市場類型自動計算完畢，直接引用數字，不得自行修改）
     違反此規則 = 把賺錢的單子放到賠錢，是最不可饒恕的交易錯誤。

  ② 若 Context 中有「獲利保護線」，優先設為移動停利的防守位。
     （確保最差也是賺 50% 利潤出場）

  ③ 零股投資人（< 1000 股）的防守線應設得更緊，
     以保護實質利潤為優先。
...
## 🎯 AI 全自動實戰交易決策：[填入公司名稱（股票代號）]

**當前股價：** [填入 {cs}X.XX] | **MA20 乖離率：** {_ma20_dev_str}
**您的持倉：** [填入 {holding_display}] | **AI 判定市場型態：** {_regime_label}
{"**目前損益：** [填入 " + pnl_display + "]" if scenario == "HOLDER" else ""}

---

### 📌 【核心行動決策】：【填入決策標題，依型態選擇：
  型態 A → 動能追擊 / 主升浪卡位 / 強勢股 MA5 蹲點
...
{_risk_header_lines}{_cost_floor_lines}

* **均線防守參考**：{cs}X.XX
  ⚠ 此防守線必須 ≥ {cs}{_price_8pct_floor}（現價 -8% 上限），低於此值的均線一律忽略
* **執行前提條件**：[型態 A：鎖在突破 K 棒低點，跌破快速出場，不等量能確認；
  型態 B：連續 2 日收盤跌破 + 量能 ≥ 均量 1.5 倍才執行；縮量跌破 = 洗盤，不執行；
  型態 C：跌破近期顯著低點或催化劑被證偽，果斷出場]
* **決策理由**：[1-2 句說明防守位依據；Hard Stop 為最終底線，所有防守線均在其之上]
...
5. 【v5.4 人性化複合風控：三道防線必須依序出現在「風險防守與認賠執行線」區塊】
   第一道 🚨 Hard Stop = {_hs_str}（現價 -6%）
     → 跌破先出 50%+，無條件，不問理由，凌駕一切
   第二道 ⚠ 成本下潛底線（依本次催化劑強度動態選取）
     → 弱催化劑（★★以下）= {_cf3_str}（成本 -3%）；強催化劑（★★★以上）= {_cf5_str}（成本 -5%）
     → 跌破此線即使洗盤三重濾網未成立也先減碼 50%
   第三道 均線防守參考（型態 B：量能條件；型態 A：K 棒低點）
     → 距現價必須在 8% 以內（即 ≥ {cs}{_price_8pct_floor}），低於此值的均線一律忽略不引用
   【絕對禁止】在報告中給出距現價 {cs}{_price} 超過 8% 以下（即 {cs}{_price_8pct_floor} 以下）的防守點
   （技術參考：MA5={cs}{technical.get('ma5','N/A')}，MA20={cs}{technical['ma20']}，MA60={cs}{technical['ma60']}，乖離率={_ma20_dev_str}）
